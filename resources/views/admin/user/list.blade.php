...
@section('pageTitle')
    用户列表<small>Control panel</small>
@endsection
@section('content')
                ...
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                        ...
                        @foreach($list as $key => $val)
                            <tr>
                                <td><img src="{{ elixir($val['profile']['avatar']) =='/' ? config('admin.uploadImage.viewParams.exampleLogo') : elixir($val['profile']['avatar']) }}" style="width: 30px;"/></td>
                                <td>{{ $val['name'] }}</td>
                                <td><a id="role_modal_name" class="btn btn-xs bg-{{ !empty($val['role']['roleName']['name']) ? 'green' : 'red' }}">{{ !empty($val['role']['roleName']['name']) ? $val['role']['roleName']['name'] : '未设置' }}</a></td>
                                <td>{{ $val['email'] }}</td>
...
@endsection
@section('script')
    @include('admin.components.modal.delete',['title' => '操作提示','content' => '你确定删除%menu_name该用户吗？'])
@endsection